...
void DISCO_USART_Init(void)
{
	...
	GPIO_InitStruct.GPIO_Pin = USART_TX_PIN | USART_RX_PIN; // Pins 6 (TX) and 7 (RX) are used
	...
	GPIO_Init(USARTPORT, &GPIO_InitStruct);					// now all the values are passed to the GPIO_Init() function which sets the GPIO registers
